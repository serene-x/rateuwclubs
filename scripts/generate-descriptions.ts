  ...
  for (let i = 0; i < needDesc.length; i += BATCH_SIZE) {
    ...
    const results = await Promise.all(
      batch.map(async (club) => {
        ...
        const prompt = `Summarize this University of Waterloo club in 1-2 sentences based on the description. If no description, use the name and tags. Stay under 150 characters. No links, emojis, or quotes.
